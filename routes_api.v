...
fn (mut app App) api_directories_get(mut ctx Context) veb.Result {
	entries := os.ls(data_dir) or {
		return ctx.handle_error(http.Status.internal_server_error, 'Could not list data directory contents', err.msg())
	}
...
fn (mut app App) api_files_directory_get(mut ctx Context, directory string) veb.Result {
	directory_path := safely_join_path(directory) or {
		return ctx.handle_error(http.Status.bad_request, 'Bad path', err.msg())
	}

	entries := os.ls(directory_path) or {
		return ctx.handle_error(http.Status.internal_server_error, 'Could not list directory contents', err.msg())
	}
...
fn (mut app App) api_files_directory_post(mut ctx Context, directory string) veb.Result {
	...
	os.mkdir(directory_path) or {
		return ctx.handle_error(http.Status.internal_server_error, 'Could not create directory', err.msg())
	}
...
fn (mut app App) api_files_directory_delete(mut ctx Context, directory string) veb.Result {
	...
	if entries.len != 0 {
		return ctx.handle_error(http.Status.bad_request, 'Directory contains ${entries.len} files', 'Refusing to delete non-empty directory')
	}
...
fn (mut app App) api_files_directory_filename_post(mut ctx Context, directory string, file_name string) veb.Result {
	...
	file_path := safely_join_path(directory, file_name) or {
		return ctx.handle_error(http.Status.bad_request, 'Bad path name', err.msg())
	}

	mut file := os.create(file_path) or {
		log.debug('failed os.create call with error: ${err.msg()}')
		return ctx.handle_error(http.Status.internal_server_error, 'Could not create file', err.msg())
	}

	file.write(ctx.req.data.bytes()) or {
		file.close()
		os.rm(file_path) or {}
		return ctx.handle_error(http.Status.internal_server_error, 'Could not write file contents', err.msg())
	}
	file.close()
	...
	gzip_file_data := gzip.compress(ctx.req.data.bytes()) or {
		os.rm(file_path) or {}
		return ctx.handle_error(http.Status.internal_server_error, 'Could not compress file', err.msg())
	}

	mut gzip_file := os.create(gzip_file_path) or {
		os.rm(file_path) or {}
		return ctx.handle_error(http.Status.internal_server_error, 'Could not create compressed file', err.msg())
	}
	...
	gzip_file.write(gzip_file_data) or {
		...
		return ctx.handle_error(http.Status.internal_server_error, 'Could not write compressed file contents', err.msg())
	}
	gzip_file.close()
...
fn (mut app App) api_files_directory_filename_delete(mut ctx Context, directory string, file_name string) veb.Result {
	...
	os.rm(file_path) or {
		return ctx.handle_error(http.Status.internal_server_error, 'Could not delete file', err.msg())
	}

	if can_gzip(file_name) {
		gzip_file_path := '${file_path}${gzip_extension}'
		os.rm(gzip_file_path) or {
			return ctx.handle_error(http.Status.internal_server_error, 'Could not delete compressed file', err.msg())
		}
	}
